---
name: autoxpp-load-integration-dev
description: >-
  Loads the Senior Integration Developer role for pre-lifecycle work:
  Azure Functions, Service Bus, Dataverse plugins, OData, integration
  architecture and debugging. No arguments — just invoke and start working.
---
...
1. Call the `autoxpp` MCP tool `get_skill_content` with `skill_name=load-integration-dev`
...
3. If the response says the skill is locked or requires an upgrade, relay that
   message to the user. Suggest running `$autoxpp-setup-api-key` if no API key
